...
PcbPtr createnullPcb()
{
    ...
    new_process_Ptr->remaining_quantum_time = 0;
...
/*******************************************************
 * PcbPtr addFrontPcb (PcbPtr* headofQ, PcbPtr process)
 *    adds the process to the front of the of the queue in 
 *
 * returns head of queue
 ******************************************************/

PcbPtr addFrontPcb(PcbPtr* q, PcbPtr p){
    if ((!q || !(*q)) && !p)
    {
        return NULL;
    }
